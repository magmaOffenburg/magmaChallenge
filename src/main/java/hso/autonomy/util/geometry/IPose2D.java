...
public interface IPose2D extends Serializable
{
	...
	/**
	 * @param other the point to check (in the same coordinate system as this)
	 * @return true if the passed point is left of this pose (>=)
	 */
	boolean isLeft(Vector2D other);
	boolean isLeft(IPose2D other);

	/**
	 * @param other the point to check (in the same coordinate system as this)
	 * @return true if the passed point is right of this pose
	 */
	boolean isRight(Vector2D other);
	boolean isRight(IPose2D other);

	/**
	 * @param other the point to check (in the same coordinate system as this)
	 * @return true if the passed point is in front of this pose (>=)
	 */
	boolean isInFront(Vector2D other);
	boolean isInFront(IPose2D other);

	/**
	 * @param other the point to check (in the same coordinate system as this)
	 * @return true if the passed point is behind this pose
	 */
	boolean isBehind(Vector2D other);
	boolean isBehind(IPose2D other);
