...
public class VectorUtils
{
	...
	public static double getDistanceBetweenXY(Vector3D a, Vector3D b)
	{
		...
		return new Vector2D(delta.getX(), delta.getY()).getNorm();
	}
	...
	/**
	 * This is effectively replacing a missing constructor of Vector2D class
	 * @param alpha the direction (in radians)
	 * @return a vector of length 1 pointing into the 2D direction of the angle
	 */
	public static Vector2D getDirectionVector(double alpha)
	{
		return new Vector2D(FastMath.cos(alpha), FastMath.sin(alpha));
	}
}