...
public class Angle implements Serializable
{
	...
	public static Angle averageArcTan(Angle[] angles)
	{
		...
		return Angle.rad(FastMath.atan2(sumSin, sumCos));
	}
	...
	{
		Vector2D rotate = rotate(position.getX(), position.getY(), angle);
		return new Vector3D(rotate.getX(), rotate.getY(), position.getZ());
	}
	...
	/**
	 * Rotates a given 2D pose by this angle.
	 *
	 * @param pose the pose to rotate
	 * @return the rotated pose
	 */
	public Pose2D applyTo(IPose2D pose)
	{
		return new Pose2D(applyTo(pose.getPosition()), pose.getAngle().add(this));
	}
	...
	/**
	 * Rotates a given 2D pose inversely by this angle.
	 *
	 * @param pose the pose to rotate
	 * @return the inversely rotated pose
	 */
	public Pose2D applyInverseTo(IPose2D pose)
	{
		return new Pose2D(applyInverseTo(pose.getPosition()), pose.getAngle().add(this.negate()));
	}
	...
	public static Angle to(Vector2D point)
	{
		...
		return new Angle(FastMath.atan2(point.getY(), point.getX()));
	}
	...
	public static Angle to(Vector2D p1, Vector2D p2)
	{
		return Angle.rad(FastMath.atan2(p2.getY() - p1.getY(), p2.getX() - p1.getX()));
	}

	public static Angle to(Vector3D p1, Vector3D p2)
	{
		return Angle.rad(FastMath.atan2(p2.getY() - p1.getY(), p2.getX() - p1.getX()));
	}

	/**
	 * Returns the angle (radians) of p1-p2 and p1-p3
	 * @param p1 the point for which to get the angle
	 * @param p2 point 2
	 * @param p3 point 3
	 * @return the angle (radians) of p1-p2 and p1-p3
	 */
	public static double getAngle(Vector2D p1, Vector2D p2, Vector2D p3)
	{
		return FastMath.atan2(p3.getY() - p1.getY(), p3.getX() - p1.getX()) -
				FastMath.atan2(p2.getY() - p1.getY(), p2.getX() - p1.getX());
	}
