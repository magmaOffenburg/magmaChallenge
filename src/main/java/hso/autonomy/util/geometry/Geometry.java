...
public class Geometry
{
	...
	public static Vector2D getPointOnOrthogonal2D(Vector2D start, Vector2D end, double distanceEnd, double distanceLeft)
	{
		...
		Vector2D orthogonal2D = getOrthogonal(start, end);
		return onLine2D.add(orthogonal2D.scalarMultiply(distanceLeft));
	}

	/**
	 * Calculates a normalized vector that is orthogonal to the vector from start to end pointing to the left resulting
	 * from a 90 degree rotation around z
	 * @param start the start point of the line
	 * @param end the end point
	 * @return the orthogonal as calculated above, (0,1) in case that start and end are too close together
	 */
	public static Vector2D getOrthogonal(Vector2D start, Vector2D end)
	{
		Vector2D endStart2D = start.subtract(end);
		if (endStart2D.getNorm() < 0.0001) {
			// avoid vector normalization exception
			return new Vector2D(0, 1);
		}
		Vector2D endStartNorm = endStart2D.normalize();
		return new Vector2D(endStartNorm.getY(), -endStartNorm.getX());
	}
	...
	{
		return new Line2D(point1, point2).getDistanceTo(thePoint);
	}
