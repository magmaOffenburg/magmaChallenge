...
public class Pose2D implements IPose2D
{
	...
	/**
	 * Creates a pose at the passed position pointing to the second position passed
	 * @param position the position of the pose
	 * @param otherPosition the position specifying to which direction we point
	 * @return a new pose 2D object
	 */
	public static Pose2D create(Vector2D position, Vector2D otherPosition)
	{
		return create(position, otherPosition, false);
	}
	 ...
	 * @param position the position of the pose
	 * @param otherPosition the position specifying to which direction we point
	 * @param reverse true, if we point away from otherPosition, false if we point to it
	 * @return a new pose 2D object
	 */
	public static Pose2D create(Vector2D position, Vector2D otherPosition, boolean reverse)
	{
		Angle angle;
		if (reverse) {
			angle = Angle.to(otherPosition, position);
		} else {
			angle = Angle.to(position, otherPosition);
		}
		return new Pose2D(position, angle);
	}
	...
	{
		return applyInverseTo(other).getY() >= 0;
	}

	@Override
	public boolean isLeft(IPose2D other)
	{
		return isLeft(other.getPosition());
	}
	...
	@Override
	public boolean isRight(IPose2D other)
	{
		return isRight(other.getPosition());
	}

	@Override
	public boolean isInFront(Vector2D other)
	{
		return applyInverseTo(other).getX() >= 0;
	}

	@Override
	public boolean isInFront(IPose2D other)
	{
		return isInFront(other.getPosition());
	}

	@Override
	public boolean isBehind(Vector2D other)
	{
		return applyInverseTo(other).getX() < 0;
	}

	@Override
	public boolean isBehind(IPose2D other)
	{
		return isBehind(other.getPosition());
	}
