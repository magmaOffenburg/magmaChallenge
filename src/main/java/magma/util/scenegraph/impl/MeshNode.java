...
public class MeshNode extends BaseNode implements IMeshNode
{
	...
	@Override
	public <T extends IBaseNode> T getNode(Class<T> nodeType, String property, String value, boolean requiresRegEx)
	{
		...
		if ("objName".equals(property)) {
			...
			} else if (requiresRegEx) {
				if (objName.matches(value)) {
					return nodeType.cast(this);
				}
			} else {
				// equals is considerably cheaper, so use it if there is no regex
