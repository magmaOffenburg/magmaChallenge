...
public class RoboVizDraw
{
	...
	public void drawPoint(String name, Vector3D position)
	{
		drawPoint(name, position, PlaySide.LEFT);
	}

	public void drawPoint(String name, Vector3D position, PlaySide side)
	{
		Color color = Color.blue;
		if (side != PlaySide.LEFT) {
			color = Color.red;
			name += "Red";
			position = new Vector3D(-position.getX(), -position.getY(), 0);
		} else {
			name += "Blue";
		}

		setCustomPrefix(name);
		drawPoint(name, position, 7, color);
	...
	public void drawMeterMarkers(boolean left, Color color, int xOffset, Angle rotation, int meters)
	{
		if (enabled) {
			...
			for (int i = 0; i < meters; i++) {
				...
				Vector3D vector = new Vector3D(x, 0, 0);
				Rotation rotation3d =
						new Rotation(new Vector3D(0, 0, 1), rotation.radians(), RotationConvention.VECTOR_OPERATOR);
				vector = rotation3d.applyTo(vector).add(new Vector3D(xOffset, 0, 0));

				String meter = Integer.toString(left ? meters - i : i + 1);
				send(newAnnotation(meter,
						new float[] {(float) vector.getX(), (float) vector.getY(), (float) vector.getZ()}, color, set));
			}
