...
public class RollingBallBenchmark extends BenchmarkMain
{
	...
	protected RunInformation createRunInformation(Random rand, int runID)
	{
		...
		Vector3D direction = angle.applyTo(new Vector3D(1, 0, 0));
		Vector3D destinationBallPosition = beamPose.getPosition().add(
				new Vector3D(0.15 + noise(rand, 0.05), 0 + noise(rand, 0.05), 0));
		Vector3D ballStart = destinationBallPosition.add(direction.scalarMultiply(radius));
